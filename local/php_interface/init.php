...
function DoIBlockAfterSave($arg1, $arg2 = false)
{
 ...
 $PRICE = $arPrice["PRICE"];
	...
	//учитываем скидки
	$prod = $arPrice['PRODUCT_ID'];
 	$arDiscounts = CCatalogDiscount::GetDiscountByProduct($arPrice['PRODUCT_ID'], $USER->GetUserGroupArray(), "N", SITE_ID);
    if(is_array($arDiscounts) && sizeof($arDiscounts) > 0) {
        $PRICE = CCatalogProduct::CountPriceWithDiscount($PRICE, $arPrice["CURRENCY"], $arDiscounts);
    }
...
if($arPrice["CATALOG_GROUP_ID"] == 4)
	if($dealerPrice === false || $dealerPrice > $PRICE)
		$dealerPrice = $PRICE;
 }

...
$arSort = Array(
	array(
		...
		'order' => 'DESC',
		'title' => 'По артикулам (По убыванию)'
	),
	array(
		'id' => 2,
		'sort' => 'PROPERTY_CML2_ARTICLE',
		'order' => 'ASC',
		'title' => 'По артикулам (По возрастанию)'
	),
	array(
		'id' => 3,
...
$curSortID = 0;
if ( $_SESSION["SORT_ID"] )
	$curSortID = $_SESSION["SORT_ID"] ;
	
$pageCountID = 0;
if ( $_SESSION["pageCountID"] )
	$pageCountID = $_SESSION["pageCountID"] ;

$pageCount = $arPageCount[$pageCountID]['value'];	
				...
				<div class="catalog__wrapper">
					...
					<div class="catalog__content">
					    ...
					    <div class="catalog__settings">
						  <div class="row">
							<div class="col-lg-6">
							  <h1 class="catalog__heading"><?=$arCurSection["NAME"]?> <small><?$APPLICATION->ShowViewContent('NavRecordCount');?></small></h1>
							  ...
							  <div class="sorting"><span class="sorting__text">Сортировать:</span>
								<div class="sorting__select">
								  <button class="sorting__select-trigger">
									...
									</svg><?=$arSort[$curSortID]['title']?>
								  </button>
								  <div class="sorting__select-dropdown">
									<?foreach ($arSort as $keySort => $arSrt ):?>
										<a class="sorting__select-option <?if ( $keySort == $curSortID):?>is-active<?endif;?>" data-sort="<?=$keySort?>" href="#"><?=$arSrt['title']?></a>
									<?endforeach;?>
									</div>
								</div>
							  </div>
							  <div class="display"><span class="display__text">Показывать:</span>
								<?foreach ( $arPageCount as $key => $arPage ):?>
									<a class="display__link page_count <?if ( $pageCountID == $key):?>is-active<?endif;?>" data-page='<?=$key?>' href="#"><?=$arPage['title']?></a><?if ( $key < count($arPageCount)-1):?>/<?endif;?>
								<?endforeach;?>
							  </div>
							</div>
						  </div>
						...
						<div class="row">
							...
							<?$intSectionID = $APPLICATION->IncludeComponent(
								...
								array(
									"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
									"IBLOCK_ID" => $arParams["IBLOCK_ID"],
									"ELEMENT_SORT_FIELD" => $arSort[$curSortID]['sort'],
									"ELEMENT_SORT_ORDER" => $arSort[$curSortID]['order'],
									"ELEMENT_SORT_FIELD2" => '',
									"ELEMENT_SORT_ORDER2" => '',
